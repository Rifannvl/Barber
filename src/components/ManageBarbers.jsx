...
const ManageBarbers = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (action === "insert") {
      ...
      query = query.update(record).eq("id", editingId);
    }

    const { error } = await query;
    if (error) {
      Swal.fire("Error", "Gagal menyimpan data kapster.", "error");
    } else {
      Swal.fire(
        "Sukses!",
  ...
  const handleDelete = async (id) => {
    ...
    if (isConfirmed) {
      ...
      if (error) {
        Swal.fire("Error", "Gagal menghapus data kapster.", "error");
      } else {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
      <div className="lg:col-span-1 bg-dark-card/50 p-6 rounded-xl border border-dark-card">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="mb-4">
            ...
            <input
              ...
              required
              className="w-full p-2 bg-dark-bg rounded border border-dark-card focus:ring-brand-blue"
            />
          </div>
          <div className="mb-4">
          ...
          <div className="flex gap-2 mt-6">
            <button
              type="submit"
              className="flex-grow p-2 bg-brand-blue text-dark-bg font-bold rounded hover:opacity-90"
            >
            ...
            {editingId && (
              <button
                type="button"
                onClick={resetForm}
                className="p-2 bg-dark-card text-light-text font-bold rounded"
              >
                Batal
              </button>
            )}
      ...
      <div className="lg:col-span-2 bg-dark-card/50 p-6 rounded-xl border border-dark-card">
            ...
            >
              <div className="flex items-center gap-4">
                <img
                  src={
                    barber.photo_url ||
                    `https://ui-avatars.com/api/?name=${barber.name}&background=00A9FF&color=1A1A1A`
                  }
                  alt={barber.name}
